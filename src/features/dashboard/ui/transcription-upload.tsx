    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <Card className="max-w-2xl mx-auto">
        ...
        <CardContent>
          <Tabs defaultValue="upload" className="space-y-6">
            ...
            <TabsContent value="upload" className="space-y-6">
              <div className="space-y-4">
                <div
                  className="border-2 border-dashed rounded-lg p-8 text-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors"
                ...
                >
                  <div className="flex flex-col items-center justify-center gap-2">
                    ...
                    <h3 className="font-medium">Drag and drop your file here or click to browse</h3>
                    <p className="text-sm text-muted-foreground">Supports MP3, WAV, M4A, FLAC (max 500MB)</p>
                    <Input
                      ref={fileInputRef}
                      type="file"
                      className="hidden"
            ...
            <TabsContent value="record" className="space-y-6">
              <div className="space-y-6">
                <div className="flex flex-col items-center justify-center gap-4 p-8 border rounded-lg">
                  ...
                  >
                    <Mic className={`h-10 w-10 ${isRecording ? "text-red-500 dark:text-red-400" : "text-slate-500"}`} />
                  </motion.div>
                  ...
                  >
                    {isRecording ? <Pause className="h-5 w-5" /> : <Play className="h-5 w-5 ml-0.5" />}
                  </Button>
                </div>
              </div>
            </TabsContent>
          ...
          <div className="space-y-4 mt-6">
            <div className="space-y-2">
              ...
              <Input id="title" placeholder="Enter a title for your transcription" />
            </div>
        ...
        <CardFooter className="flex justify-between border-t p-6">
          ...
          <Button onClick={handleUpload} disabled={!selectedFile && !isRecording}>
